...
<div class="justify-center max-w-sm w-full lg:max-w-full lg:flex mt-4">
  <div class="border border-gray-300 bg-white rounded p-4 flex flex-col leading-normal">
    ...
    <h3 class="font-bold text-base text-gray-800 mb-1">Create new User</h3>
    <hr/>
    ...
    <form action="{{ route('user.update', $user->id) }}" method="POST">
        ...
        <div class="grid grid-rows-1 grid-cols-2 gap-4 mt-3">
            <div>
                ...
                <x-input-text :name="'firstName'" :value=" $user->firstName "/>
            </div>
            <div>
                <x-label :for="'surname'">Surname</x-label>
                <x-input-text :name="'surname'" :value=" $user->surname "/>
            </div>
        </div>
        @error('firstName')
        ...
        <div class="mt-5 mb-5">
            ...
            <x-input-text :name="'email'" :value=" $user->email "/>
        </div>
        ...
        <div class="mb-5">
            ...
            @foreach($groups as $group)
                <div>
                    <x-input-checkbox :id=" $group->id " :name="'groups[]'" :value=" $group->id " :checked=" $checked[$group->id] "/>
